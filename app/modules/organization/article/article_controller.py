...
from app.modules.organization.article import article_dto
from app.modules.organization.organization_controller import Organization
...
class ArticleController(Controller):
    ...
    def create(self, data):
        ...
        if not 'organization_id' in data:
            return send_error(message=messages.ERR_PLEASE_PROVIDE.format('organization_id'))
        
        data['entity_type'] = 'organization'
        data['is_draft'] = 1
        
        organization = Organization.query.filter_by(id=data['organization_id']).first()
    ...
    def update_status(self, object_id, data):
        try:
            ...
            if data['status'] == 'approved':
                ...
                article.is_draft = expression.true()
                article.published_date = None
            else:
                return send_error(message=messages.ERR_ISSUE.format('status must be: approved or drafted'))
            db.session.commit()
            return send_result(message=messages.MSG_UPDATE_SUCCESS, data=marshal(article, ArticleDto.model_article_response))
        except Exception as e:
            db.session.rollback()
            print(e.__str__())
    ...
    def update(self, object_id, data):
        ...
        if article.entity_type == 'organization' and article.organization_id is None:
            return send_error(message=messages.ERR_PLEASE_PROVIDE.format(' organization_id when provided entity_type is organization!'))
        if article.organization_id is not None:
            organization = Organization.query.filter_by(id=article.organization_id).first()
            if organization is None:
                return send_error(message=messages.ERR_NOT_FOUND_WITH_ID.format('Organization', article.organization_id))
        try:
    ...
    def _parse_article(self, data, article=None):
        ...
        if 'entity_type' in data:
            try:
                article.entity_type = data['entity_type']
            except Exception as e:
                print(e.__str__())
                pass
        if 'organization_id' in data:
            try:
                article.organization_id = data['organization_id']
            except Exception as e:
                print(e.__str__())
                pass
