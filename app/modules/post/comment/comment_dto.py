...
class CommentDto(Dto):
    name = 'post_comment'
    api = Namespace(name, description="Post comment operations")

    comment_user = api.model('post_comment_user', {
    ...
    model_request = api.model('post_comment_request', {
        'comment': fields.String(required=True, description='The content of the comment')
    })

    model_response = api.model('post_comment_response', {
