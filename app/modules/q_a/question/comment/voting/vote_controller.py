...
class QuestionCommentVoteController(Controller):
    def get(self, args, comment_id = None):
        ...
        query = QuestionCommentVote.query
        if user_id is not None:
            query = query.filter(QuestionCommentVote.user_id == user_id)
        if comment_id is not None:
            query = query.filter(QuestionCommentVote.comment_id == comment_id)
        if from_date is not None:
            query = query.filter(QuestionCommentVote.created_date >= from_date)
        if to_date is not None:
            query = query.filter(QuestionCommentVote.created_date <= to_date)
        votes = query.all()
        if votes is not None and len(votes) > 0:
            return send_result(data=marshal(votes, QuestionCommentVoteDto.model_response), message='Success')
        else:
            return send_result(message='QuestionComment vote not found')

    def get_by_id(self, object_id):
        if id is None:
            return send_error(message='Please provide id')
        vote = QuestionCommentVote.query.filter_by(id=object_id).first()
        if vote is None:
            return send_error(message='QuestionComment vote not found')
        else:
    ...
    def create(self, comment_id, data):
        ...
        try:
            ...
            vote = QuestionCommentVote.query.filter(QuestionCommentVote.user_id == data['user_id'], \
                QuestionCommentVote.comment_id == data['comment_id']).first()
            if vote:
    ...
    def delete(self, comment_id):
        ...
        try:
            vote = QuestionCommentVote.query.filter_by(comment_id=comment_id, user_id=user_id).first()
            if vote is None:
                return send_error(message='QuestionComment vote not found')
            else:
                db.session.delete(vote)
                db.session.commit()
                return send_result(message='QuestionComment vote deleted successfully')
        except Exception as e:
            print(e.__str__())
