...
from app.modules.q_a.comment.voting.vote import CommentVote, VotingStatusEnum
...
class CommentVoteController(Controller):
    def get(self, args, comment_id = None):
        ...
        query = CommentVote.query
        if user_id is not None:
            query = query.filter(CommentVote.user_id == user_id)
        if comment_id is not None:
            query = query.filter(CommentVote.comment_id == comment_id)
        if from_date is not None:
            query = query.filter(CommentVote.created_date >= from_date)
        if to_date is not None:
            query = query.filter(CommentVote.created_date <= to_date)
        votes = query.all()
        if votes is not None and len(votes) > 0:
            return send_result(data=marshal(votes, CommentVoteDto.model_response), message='Success')
        else:
            return send_result(message='Comment vote not found')

    def get_by_id(self, object_id):
        if id is None:
            return send_error(message='Please provide id')
        vote = CommentVote.query.filter_by(id=object_id).first()
        if vote is None:
            return send_error(message='Comment vote not found')
        else:
            return send_result(data=marshal(vote, CommentVoteDto.model_response), message='Success')
    ...
    def create(self, comment_id, data):
        ...
        try:
            ...
            vote = CommentVote.query.filter(CommentVote.user_id == data['user_id'], \
                CommentVote.comment_id == data['comment_id']).first()
            if vote:
    ...
    def delete(self, comment_id):
        ...
        try:
            vote = CommentVote.query.filter_by(comment_id=comment_id, user_id=user_id).first()
            if vote is None:
                return send_error(message='Comment vote not found')
            else:
                db.session.delete(vote)
                db.session.commit()
                return send_result(message='Comment vote deleted successfully')
        except Exception as e:
            print(e.__str__())
