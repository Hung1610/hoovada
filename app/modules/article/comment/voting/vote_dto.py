...
class ArticleCommentVoteDto(Dto):
    name = 'article_comment_vote'
    api = Namespace(name, description="ArticleComment voting operations")

    model_request_comment = api.model('article_vote_request_comment_vote', {
        'vote_status': fields.Integer(description='1 - Neutral, 2 - Upvote, 3 - Downvote', default=False)
    })

    model_response = api.model('article_vote_response_comment_vote', {
