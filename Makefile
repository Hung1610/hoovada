...
deploy:
	...
	#@kubectl set image deployment/dramatiq dramatiq=${DRAMATIQ_TEST} nginx=${NGINX_TEST} -n hoovada-staging --record
...
deploy-live:
	...
	@kubectl set image deployment/dramatiq dramatiq=${DRAMATIQ_LIVE} nginx=${NGINX_TEST} -n hoovada-staging --record
